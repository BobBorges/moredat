...
from .forms import DeleteWikiPageForm, WikiPageForm
...
def index(request):
    ...
    """
    kwargs = {'slug': getattr(settings, 'DOCUMENTATION_DEFAULT_INDEX', 'WikiIndex')}
    redirect_to = reverse('documentation_page', kwargs=kwargs)
    return HttpResponseRedirect(redirect_to)


def page(
    request, slug, rev_id=None, template_name='documentation/page.html', extra_context=None,
):
    """
    ...
    # deny, if the user has no permission to add pages
        ...
        if request.user.is_authenticated:
            kwargs = {'slug': slug}
            redirect_to = reverse('documentation_edit', kwargs=kwargs)
            return HttpResponseRedirect(redirect_to)
        raise Http404
...
def edit(
    ...
    slug,
    rev_id=None,
    template_name='documentation/edit.html',
    extra_context=None,
    ...
    try:
        ...
        if not request.user.has_perms(
            ('documentation.change_wikipage', 'documentation.change_revision')
        ):
    ...
    # Note that it's not saved here
        ...
        if not request.user.has_perms(
            ('documentation.add_wikipage', 'documentation.add_revision')
        ):
            return HttpResponseForbidden(
    ...
    if request.user.has_perm('documentation.delete_wikipage') or request.user.has_perm(
        'documentation.delete_revision'
    ):
...
def revisions(
    request, slug, template_name='documentation/revisions.html', extra_context=None
):
    """
...
def changes(request, slug, template_name='documentation/changes.html', extra_context=None):
    """
...
# Some useful views
def revision_list(
    request, template_name='documentation/revision_list.html', extra_context=None
):
    """
...
def page_list(request, template_name='documentation/page_list.html', extra_context=None):
    """
    Displays all Pages
    """
    ...
    template_context = {
        'page_list': page_list,
        'index_slug': getattr(settings, 'DOCUMENTATION_DEFAULT_INDEX', 'WikiIndex'),
    }
    template_context.update(extra_context or {})
    return render(request, template_name, template_context)
