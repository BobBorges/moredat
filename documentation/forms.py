...
class DeleteWikiPageForm(forms.Form):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        if request.user.has_perm('documentation.delete_revision'):
            self.base_fields['delete'].choices.append(
                ('rev', _('Delete this revision'))
            )

        if request.user.has_perm('documentation.delete_revision') and request.user.has_perm(
            'documentation.delete_wikipage'
        ):
            self.base_fields['delete'].choices.append(
    ...
    def delete_wiki(self, request, page, rev):
        ...
        if (
            ...
            and request.user.has_perm('documentation.delete_revision')
            and request.user.has_perm('documentation.delete_wikipage')
        ):
            self._delete_page(page)
            messages.success(request, gettext('The page %s was deleted' % page.slug))
            return HttpResponseRedirect(reverse('documentation_index'))
        ...
        if self.cleaned_data.get('delete') == 'rev':
            ...
            if revision_length > 1 and request.user.has_perm(
                ...
                return HttpResponseRedirect(
                    reverse('documentation_page', kwargs={'slug': page.slug})
                )
